...
export class UserTokenStrategy implements TokenStrategy {
  ...
  async refreshToken(tenantId: string, userId: string): Promise<string | null> {
    ...
    try {
      ...
      if (!tenantSettings?.secret?.logInFlow) {
        console.error(
          `❌ [UserTokenStrategy] No cached tenant settings with logInFlow for tenant ${tenantId}`
        );
        return null;
      }
      ...
      if (!clientId || !clientSecret) {
        console.error(
          `❌ [UserTokenStrategy] Missing logInFlow credentials for tenant ${tenantId}`
        );
        return null;
      }

      console.log(
        `🔄 [UserTokenStrategy] Refreshing user token with logInFlow clientId: ${clientId.substring(
          0,
          10
        )}...`
      );
  ...
  async getUserAccessToken(
    ...
    return await this.cache.get<string>(
      CacheKeys.userAccessToken(tenantId, userId)
    );
  }
  ...
  async getValidUserAccessToken(
    ...
    if (ttl <= this.accessTokenConfig.safetyMargin) {
      console.log(
        `⏰ UserToken (${userId}): Token expiring soon (${ttl}s remaining, safety margin: ${this.accessTokenConfig.safetyMargin}s), clearing`
      );
    ...
    console.log(
      `✅ UserToken (${userId}): Valid cached token found (${ttl}s until expiry)`
    );
    return currentToken;
  }
  ...
  async setUserAccessToken(
    ...
    console.log(
      `💾 UserAccessToken (${userId}): Caching token with Redis TTL: ${redisTTL}s (token lifetime: ${expiresIn}s)`
    );
  ...
  async getUserRefreshToken(
    ...
    const stored = await this.cache.get<StoredToken>(
      CacheKeys.userRefreshToken(tenantId, userId)
    );
    if (!stored) {
      return null;
    }
  ...
  async setUserRefreshToken(
    ...
    console.log(
      `💾 UserRefreshToken (${userId}): Caching token with Redis TTL: ${redisTTL}s (token lifetime: ${expiresIn}s)`
    );
    await this.cache.set(
      CacheKeys.userRefreshToken(tenantId, userId),
      storedToken,
