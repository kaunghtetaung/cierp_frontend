...
/**
 * um1sf post hero — themed band echoing `PageHeader`:
 *
 * - With featured image: image fills the band as background, dark
 *   gradient overlay for legibility, white text.
 * - Without featured image: solid `--color-primary` band with a
 *   single medicine-sign watermark on the right (matches Page
 *   Header), `--color-primary-foreground` text.
 *
 * Typography uses `um1sf-hero-headline` for consistency with
 * PageHeader. Vertical rhythm matches PageHeader (py-12 → 16 → 20)
 * so news/event pages and About/Faculty pages share the same chrome
 * weight.
 */
export function PostHero({
  ...
  return (
    ...
    >
      {hasImage ? (
        <>
          {/* Featured image as background. eslint-disable: dynamic
              tenant-storage URLs aren't enumerable for next.config. */}
        ...
        // Medicine-sign watermark — only when no featured image, so
        // the band has visual identity instead of a flat colour wall.
        <svg
          aria-hidden
      ...
      <div
        className={`relative max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 lg:py-20 ${
          !hasImage ? "lg:pr-56" : ""
        }`}
      >
        {renderCrumbs && (
        ...
        {renderTitle && (
          <div className="w-full">
            {title && (
              // `leading-relaxed` (~1.625) gives Myanmar diacritics
              // enough vertical room so stacked marks don't collide
              // with the line above; `leading-snug` was too tight.
              // Title size caps at `lg:text-2xl` (24px) — `text-3xl`
              // on xl screens read as a banner shout, not a heading.
              <h1 className="um1sf-hero-headline text-lg md:text-xl lg:text-2xl font-bold leading-loose tracking-tight">
                {title}
              </h1>
            )}
            {excerpt && (
