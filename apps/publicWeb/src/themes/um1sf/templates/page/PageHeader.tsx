...
interface PageHeaderProps {
  title?: string;
  description?: string;
  /** Crumb chain — already localized + ordered Home → … → current. Last item = current page (no link). */
...
/**
 * um1sf page header — themed band with a subtle medical / caduceus
 * pattern, optional breadcrumbs, optional title + description.
 *
 * Render rules (matches author intent):
 *   - Both flags off → render nothing.
 *   - Either flag on  → render the band; show only what's enabled.
 *   - Breadcrumbs sit *above* the title inside the same band, so
 *     authors get one cohesive header rather than two stacked
 *     blocks. The band's themed color provides the "page chrome"
 *     look the rest of um1sf relies on.
 */
export function PageHeader({
  ...
  const renderTitle = showTitle && (title || description);
      ...
      <div className="relative max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 lg:py-20 lg:pr-56">
                ...
                return (
                  <li key={`${i}-${c.label}`} className="flex items-center gap-1.5">
                    ...
                    {isLast || !c.href ? (
                      ...
                      >
                        {i === 0 && <Home className="h-3.5 w-3.5 opacity-90" />}
                        {c.label}
                      </span>
                    ) : (
                      <Link
        ...
        {renderTitle && (
          <div className="text-center md:text-left">
            {title && (
              <h1 className="um1sf-hero-headline text-3xl md:text-4xl lg:text-5xl font-bold leading-tight tracking-tight">
                {title}
              </h1>
            )}
            {description && (
              <p
                className="mt-4 md:mt-5 text-base md:text-lg max-w-2xl opacity-90 leading-relaxed"
