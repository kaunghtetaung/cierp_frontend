...
export async function HeaderDefault({
  ...
  // ── Fetch settings first to learn which menuType slugs to query.
  // React.cache deduplicates the call so other variants in the same
  // render get the same instance for free.
  ...
  const secondaryMenuType =
    (settings as any)?.header?.secondaryMenuType || "secondary-header-menu";

  const [primaryMenu, audienceMenu] = await Promise.all([
    fetchMenuTree(primaryMenuType, tenantId).catch(
      () => [] as MenuTreeNode[],
    ),
    fetchMenuTree(secondaryMenuType, tenantId).catch(
      () => [] as MenuTreeNode[],
    ),
  ]);
...
async function fetchMenuTree(
  ...
  try {
    ...
    if (!response?.success) {
      console.warn(
        `HeaderDefault: menu ${menuType} fetch failed:`,
        response?.error,
      );
      return [];
    }
    ...
    console.warn(
      `HeaderDefault: failed to fetch menu ${menuType}`,
