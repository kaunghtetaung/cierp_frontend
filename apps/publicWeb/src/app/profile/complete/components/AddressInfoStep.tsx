...
export function AddressInfoStep() {
  ...
  const [stateRegions, setStateRegions] = useState<LocationOption[]>([]);
  const [districts, setDistricts] = useState<LocationOption[]>([]);
  const [townships, setTownships] = useState<LocationOption[]>([]);
              ...
              >
                ...
                <SelectContent className="bg-white border border-gray-300 max-h-[300px]">
                  {stateRegions.map((state) => (
                    <SelectItem key={state.pcode} value={state.name}>
                      {state.name}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              >
                ...
                <SelectContent className="bg-white border border-gray-300 max-h-[300px]">
                  {districts.map((district) => (
                    <SelectItem key={district.pcode} value={district.name}>
                      {district.name}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              >
                ...
                <SelectContent className="bg-white border border-gray-300 max-h-[300px]">
                  {townships.map((township) => (
                    <SelectItem key={township.pcode} value={township.name}>
                      {township.name}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          <Controller
            ...
            render={({ field }) => (
              ...
              >
                ...
                <SelectContent className="bg-white border border-gray-300 max-h-[300px]">
                  {towns.map((town) => (
                    <SelectItem key={town.pcode} value={town.name}>
                      {town.name}
                    </SelectItem>
                  ))}
                </SelectContent>
