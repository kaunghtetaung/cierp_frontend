// Library layout data fetching utilities
// Extends baseDashboard utilities with library-specific functionality
...
// Create error handlers for library context
const { handleTenantError, validateTenant } = createTenantErrorHandler("library")
const { handleSchemaError, logSchemaSuccess } = createAppSchemaErrorHandler("library")
...
/**
 * Generate metadata for the library application
 */
export function generatePageMetadata(tenant: TenantSettings | null): PageMetadata {
  return generateDashboardMetadata(tenant, "Library")
}