...
// Get accession groups
export async function getAccessionGroups() {
  try {
    const moduleService = await createModuleService();

    const endpoint = `/library/accession-groups`;
    ...
    if (!response.success) {
      throw new Error(response.error || "Failed to fetch accession groups");
    }
    ...
    console.error("Failed to get accession groups:", error);
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to fetch accession groups",
      data: []
    };
  }
}
