...
export class MultilingualErrorBoundary extends Component<
  MultilingualErrorBoundaryProps,
  MultilingualErrorBoundaryState
> {
  ...
  static getDerivedStateFromError(
    error: Error
  ): Partial<MultilingualErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError && this.state.error) {
      ...
      const recoveryActions = getLocalizedRecoveryActions(
        errorMessageKey,
        language
      );
          ...
          <div className="max-w-md w-full text-center">
                ...
                <ul className="text-xs text-muted-foreground space-y-1">
                  {recoveryActions.map((action, index) => (
                    <li key={index} className="flex items-start gap-2">
                      <IconComponent
                        name="ArrowRight"
                        className="w-3 h-3 mt-0.5 text-muted-foreground/60"
                      />
                      {action}
                    </li>
                  ))}
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {language === "mm"
                  ? "စာမျက်နှာကို ပြန်လည်ရှင်းသန့်စေပါ"
                  : "Reload Page"}
              </Button>
            </div>
          </div>
...
export function withMultilingualErrorBoundary<P extends object>(
  ...
  WrappedComponent.displayName = `withMultilingualErrorBoundary(${
