  ...
  return withApiErrorHandler(request, async (req) => {
    ...
    if (!languageService.isValidLanguage(language)) {
      return NextResponse.json({
        success: false,
        language: language,
        error: `Invalid language code: ${language}. Supported languages: ${languageService.getSupportedLanguages().map((l: any) => l.code).join(', ')}`
      }, { status: 400 });
    }
    ...
    const result = {
      success: true,
      language: language,
      message: `ERP language changed to ${language}`
    };
    ...
    response.cookies.set(config.cookieName!, language, {
      ...
      sameSite: config.cookieOptions!.sameSite as any,
